...
class Cart(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)  # Connect the cart to a user
...
class CartItem(models.Model):
    cart = models.ForeignKey(Cart, on_delete=models.CASCADE)  # Connect the item to a cart
    dish = models.ForeignKey(Dish, on_delete=models.CASCADE)  # Connect the item to a dish
    quantity = models.PositiveIntegerField(default=1)  # Track the quantity of each item

    def total_price(self):
        return self.quantity * self.dish.discounted_price  # Calculate the total price for the item

    def __str__(self):
        return f"{self.quantity} of {self.dish.name} in cart {self.cart.id}"
